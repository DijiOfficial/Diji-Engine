...
#include "Collision.h" // for RectI

using namespace diji;
bool SVGParser::GetVerticesFromSvgFile( const std::string& filePath, std::vector<std::vector<glm::vec2>> &vertices, const int yAdjust)
{
	const std::string fullpath = "../Data/" + filePath;
	// Open the file
	std::ifstream svgStream(fullpath.c_str( ) );
...
bool SVGParser::GetVerticesFromSvgString(std::string& svgString, std::vector<std::vector<glm::vec2>> &vertices)
{
	...
	{
		// Vector of glm::vec2 to fill with a path's vertices
...
bool SVGParser::GetVerticesFromPathData( const std::string& pathData, std::vector<glm::vec2> &vertices )
{
	...
	char cmd{ 0 };
	glm::vec2 cursor{};
	glm::vec2 startPoint{};//At the end of the z command, the new current point is set to the initial point of the current subpath.
